...
reordering the loops for the thomas solver of y and z axes. (see the x/y/z loops in solve_slice_y and solve_slice_z)
- Better fectorization for small enough number of substrates. Since compiler generated vectorization is done over the
...
the whole 'plane' s*x rather than just s. For it to work properly, helper vectors b, c, e had to be modified such their
data span over x*s, not only over s (so they got copied x times in the suitable dimension).
*/
...
class diffusion_solver
{
	...
	void precompute_values_times_x(std::unique_ptr<real_t[]>& b, std::unique_ptr<real_t[]>& c,
...
public:
	...
	void initialize(microenvironment& m, bool help_vectorize_yz);
