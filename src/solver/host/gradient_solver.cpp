...
#pragma omp for nowait
	...
	solve_z_omp(gradients, densities, voxel_shape[2], dens_l, grad_l ^ noarr::fix<'d'>(noarr::lit<2>));
}
...
#pragma omp for nowait
	...
	solve_y_omp(gradients, densities, voxel_shape[1], dens_l, grad_l ^ noarr::fix<'d'>(noarr::lit<1>));
}
