...
auto fix_dims(const real_t* cell_position, const cartesian_mesh& m)
{
	if constexpr (dims == 1)
	{
		point_t<index_t, 3> voxel_index = m.voxel_position<1>(cell_position);
		return noarr::fix<'x'>(voxel_index[0]);
	}
	else if constexpr (dims == 2)
	{
		point_t<index_t, 3> voxel_index = m.voxel_position<2>(cell_position);
		return noarr::fix<'x'>(voxel_index[0]) ^ noarr::fix<'y'>(voxel_index[1]);
	}
	else if constexpr (dims == 3)
	{
		point_t<index_t, 3> voxel_index = m.voxel_position<3>(cell_position);
